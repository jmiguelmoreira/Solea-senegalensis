function  test_get_LW() 
% testing; more complex version with get_ELH_j (maturity is tracked  in case f was not constant)
% make sure you have pars_Solea.mat in same directory
...
%   options = odeset('Events', @event_bj); % capture events - birth and metamorphosis
  ...
  [t, ELH, te, ye, ie] = ode45(@get_ELH_j, linspace(time(1),time(end),1e3), ELH_h, options, f_Man, v, g, E_m, L_m, p_Am, tTC, kap, k_J, E_Hb, E_Hj); % ELH: {J/cm^3, cm}, with {[E], L, H}
...
options = odeset('AbsTol',1e-8, 'RelTol',1e-6, 'Events',@event_bj); % increase integration sensitivity; capture events - birth and metamorphosis
 [t, EL, te, ye, ie] = ode45(@get_EL_j, [0 ; tL2(:,1)], EL_b, options, f_RibeEngr, v, g, E_m, L_m, p_Am, TC_tL2, kap, k_J, lb*L_m, lj*L_m); % ELH: {J/cm^3, cm}, with {[E], L, H}
 EL(1,:) = [];
 ELw2 = EL(:,2) / del_M;
...
function [dELH, sM]  = get_ELH_j(t, ELH, v, g, E_m, L_m, p_Am, kap, k_J, E_Hb, E_Hj, f, tTC)
...
function [value,isterminal,direction] = event_bj(t, ELH, v, g, ~, L_m, p_Am, kap, k_J, E_Hb, E_Hj, f, tTC)
